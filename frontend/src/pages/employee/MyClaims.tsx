...
const ALL_STATUSES: ClaimStatus[] = [
  'DRAFT', 'SUBMITTED', 'CHANGES_REQUESTED', 'APPROVED', 'REJECTED', 'PAID', 'WITHDRAWN',
];
      ...
      <div className="card">
        ...
        {loading ? (
          ...
          <div className="empty-state">
            ...
            <p>{filter === 'ALL' ? 'No claims yet. Create your first claim.' : `No ${filter.replace('_', ' ').toLowerCase()} claims.`}</p>
          </div>
        ) : (
              ...
              <tbody>
                {filtered.map((claim) => (
                  <tr
                    ...
                    onClick={() => {
                      if (claim.status === 'DRAFT' || claim.status === 'CHANGES_REQUESTED') {
